if __name__ == "__main__":
    ...
    for variant, k_dim in VARIANTS.items():
        ...
        engine = KyberCoreEngine(N_PARAM, Q_PARAM, ETA_PARAM,
                                 k_dimension=k_dim, d_u=du, d_v=dv)
        ...
        for label, msg_str in TEST_MESSAGES.items():
            ...
            # Use encrypt_chunked / decrypt_chunked for arbitrary-length messages
            t0 = time.perf_counter()
            ciphertexts = engine.encrypt_chunked(pk, msg_str)
            t_enc = time.perf_counter() - t0

            t0 = time.perf_counter()
            recovered = engine.decrypt_chunked(sk, ciphertexts, output_type="str")
            t_dec = time.perf_counter() - t0

            ok = "✓ PASS" if recovered == msg_str else "✗ FAIL"
            print(f"\n  [{label}]  chunks={len(ciphertexts)}  {ok}")
    ...
    engine = KyberCoreEngine(N_PARAM, Q_PARAM, ETA_PARAM, k_dimension=2, d_u=10, d_v=4)
    pk, sk = engine.keygen(os.urandom(32))
